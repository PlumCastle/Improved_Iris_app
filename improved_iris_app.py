...
rfe_model = RandomForestClassifier(n_jobs = -1,n_estimators = 100)
...
#variable_name = st.sidebar.selectbox(label,tuple)
...
if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    ...
    pred_score = svc_model.score(X_train,y_train)
  elif classifier == 'Logistic Regression':
    species_type = prediction(log_reg, s_length, s_width, p_length, p_width)
    pred_score = log_reg.score(X_train,y_train)
  elif classifier == "Random Forest Classifier":
    species_type = prediction(rfe_model, s_length, s_width, p_length, p_width)
    pred_score = rfe_model.score(X_train,y_train)
  st.write("species predicted was:",species_type)
  st.write("score of the model was:",pred_score)